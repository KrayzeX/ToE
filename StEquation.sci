...
//и контурные токи; возвращает матрицу коэффициентов уравнений состояния

function [result, CR] = StEquation(LoopMatrix, BranchResistance, EMFMatrix, CurrentMatrix, ReactType, ValueBranch1, ValueBranch2)
    ...
    if ReactType == 1 then
        ...
        //Ищем контур с L-элементом с ненулевым током
        LLoop = 1;
        while (LLoop <= rows) && ((LoopMatrix(LLoop, Branch) == 0) || ((Summ(LLoop, 1) == 0) && (Summ(LLoop, 2) == 0)) && (Summ(LLoop, 3) == 0))
            LLoop = LLoop + 1;
        end;
